...
def rate_individual(parent, _gene):
    ...
    fitness = mean(list(map(operator.pow,list(map(operator.sub, results, target_values)),itertools.repeat(2,len(results)))))
...
if __name__ == "__main__":
    ...
    population_size = 100
    starting_individual_length = 10
    max_individual_length = 40
    max_generations = 5000

    # terminal sets
    t = [['x', 'y'],
         ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'math.pi', 'math.e']]
    # function sets
    f = [['+', '-', '*', '/'], # arithmetic
         # ['&', '|'], #bitwise
         ['math.sin', 'math.cos', 'math.tan'], # periodic
         ['math.log', 'math.sqrt']]
    # arity
    _a = [[2, 2, 2, 2],
         # [2, 2],
    ...
    objective_function = "(math.tan(x) - math.tan(y))/(1+math.tan(x)*math.tan(y))"
